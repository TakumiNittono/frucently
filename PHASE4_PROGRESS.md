...
### 4. 会話履歴保存
- [x] メッセージの保存（基本実装完了）
  - [x] ユーザーメッセージの保存（ローカルストレージ）
  - [x] AI応答の保存（ローカルストレージ）
  - [x] 会話IDの管理（基本実装）
- [ ] ベクトル埋め込み生成・保存（将来の最適化）
  - [ ] OpenAI Embeddings APIの統合
  - [ ] メッセージの埋め込み生成
  - [ ] ベクトルの保存

**メモ**:  
基本的な会話履歴保存機能を実装しました。現在はローカルストレージを使用していますが、後でSupabaseに移行可能な設計になっています。`app/lib/conversation.ts`で会話履歴管理ユーティリティを実装済みです。  
...
**メモ**:  
プロンプトへの動的注入機能を実装しました。最新10件の会話履歴を自動的にプロンプトに注入し、AIが文脈を理解できるようにしました。`app/api/voice/route.ts`で会話履歴をメッセージに追加する実装を完了しました。  
